        ...
        <div class="form-heading">
            <div class="container">
                ...
                <form action="home.php" method="POST" class="form-inline">
                    ...
                    <div class="form-group mx-sm-3 mb-2">
                        <div class="form-group">
                            <select name="services" class="form-control" required>
                                <?php $query = "select kategori from services";
                    ...
                    <button type="submit" class="btn btn-success mb-2 btn-login" name="date_search">
                        Cari
                    </button>
                </form>
                ...
                if (isset($_POST['date_search'])) {
                    ...
                    $available = true;
                    $query = "select * from booking where tgl = '$date' and services = '$services'";
                    $query_run = mysqli_query($db_connection, $query);
                    while ($row = mysqli_fetch_assoc($query_run)) {
                        if ($row['tgl']  ==  $available) {
                            $available = false;
                        }
                    }
                    if ($day + 1 < 0 || $day + 1 > 7) {
                ...
                if ($available) {
                        header("location: searchbooking.php?date=" . $date . "&services=" . $services);
                    } else {
                        echo '
            <div class="alert alert-danger mt-3 mb-1" style="max-width: 50%" role="alert">
            <h5 class="alert-heading">Tanggal tidak tersedia</h5>
            Harap masukkan tanggal lain.
            </div>
            ';
                    }
                    echo '</div>';
